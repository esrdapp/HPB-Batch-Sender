<template>
  ...
  <p>
    Enter HPB destination addresses with amounts, seperated by a semi-colon. One per line:<br/>
    <textarea id="dests" v-model="destinationsCsv"
              placeholder="0x9b14c5fb31de30246218d77b1b3a30a22714cfc7;0.1	0x172b49140d31a034be7f358fddd9db25a71a402a;0.123	0xdBCBA6C9D5edBD9C110c90556035E3aA7e2E2C0e;2.15		0x375fd6a91f30f812c8a4039392a8f0e093ca2dfc;0.152">
	</textarea>
  ...
  <p v-if="!metamaskConnected">
    <span>You are not connected to Metamask! </span>
  ...
  <p v-if="transactionId" class="txid">
	...
	<span v-if="eth && eth.chainId==='0x10D'">
  ...
  <div v-if="destinationList.length>0">
    <div v-if="destinationErrorCount === 0 && !destinationsCheckedOnChain && metamaskConnected">
      <button @click="checkAddressesForContracts">
        (Step 2): Check if addresses are not smart contracts and have been used before
      </button>
    </div>
    ...
    <div v-if="senderAccount" class="refundAddress">
      Refund address where remaining HPB will go back to: {{ senderAccount }}
    </div>
    ...
    <p v-if="destinationList.length < 4" class="error">
      ...
      4 destinations, otherwise it won't be cheaper than individual transactions.
    </p>

    <p v-if="destinationErrorCount > 0" class="error">
      {{ destinationErrorCount }} of the lines contain some kind of syntax error. Please correct them before you
      continue!
    </p>
...
// all the remaining HPB go back to the sending address
      ...
      function successCallback(result) {
        ...
        try {
          if (result && result.length > 0 && typeof result[0] === 'string' && self.assertAddressWithPrefix(result[0])) {
            ...
            self.messages = 'You are now connected to Metamask :)';
